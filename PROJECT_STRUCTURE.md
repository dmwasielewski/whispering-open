...
- `packages/auth`, `packages/encryption`, `packages/sync` - undesirable long term, present because dependency chains still reference them.
...
4. Audit and remove unused `packages/auth`, `packages/sync`, and `packages/encryption` dependencies.
